...
function sumStats(list: any[]) {
  return Object.fromEntries(STAT_KEYS.map(key => [key, sum(list.map(n => n?.[key]))])) as any;
}
...
function sumSeries(series: { x: string; y: number }[][]) {
  ...
  series.flat().forEach(({ x, y }) => {
    totals.set(x, (totals.get(x) || 0) + (+y || 0));
  });
...
export function useOverviewData() {
  ...
  const results = useQueries({
    queries: websites.map((website: any) => ({
      queryKey: ['overview:website', { websiteId: website.id, startAt, endAt, unit, timezone }],
      queryFn: async (): Promise<OverviewSite> => {
  ...
  const sites = results.map(r => r.data).filter(Boolean) as OverviewSite[];
